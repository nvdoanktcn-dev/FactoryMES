from __future__ import annotations

from src.services.employee_service import (
    EmployeeService,
)
from src.services.master_import.import_detail_service import (
    ImportDetailService,
)
from src.services.master_import.import_engine import (
    ImportEngine,
    ImporterRegistry,
)
from src.services.master_import.importers import (
    GenericMasterImporter,
)
from src.services.master_import.mappers import (
    EmployeeMapper,
)
from src.services.master_import.runtime.employee_import_config import (
    database_employee_to_dict,
    employee_entity_key,
    employee_entity_to_service_data,
)
from src.services.master_import.transaction import (
    SQLAlchemyTransactionManager,
)


def build_employee_import_engine(
    session,
) -> ImportEngine:
    if session is None:
        raise ValueError(
            "SQLAlchemy session is required."
        )

    employee_service = EmployeeService(
        session=session
    )

    detail_service = ImportDetailService(
        session=session,
        auto_commit=False,
    )

    importer = GenericMasterImporter(
        module_name="EMPLOYEE",
        mapper=EmployeeMapper(),
        service=employee_service,
        save_method=(
            employee_service.save_employee
        ),
        get_method=(
            employee_service.get_employee
        ),
        entity_key_getter=(
            employee_entity_key
        ),
        entity_to_service_data=(
            employee_entity_to_service_data
        ),
        database_entity_to_dict=(
            database_employee_to_dict
        ),
        import_detail_service=(
            detail_service
        ),
    )

    registry = ImporterRegistry()
    registry.register(
        importer
    )

    transaction_manager = (
        SQLAlchemyTransactionManager(
            session=session,
            close_on_finish=False,
        )
    )

    return ImportEngine(
        registry=registry,
        transaction_manager=(
            transaction_manager
        ),
    )