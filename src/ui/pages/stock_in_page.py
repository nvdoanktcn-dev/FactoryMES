...
from src.ui.dialogs.stock_in_dialog import StockInDialog
from src.ui.framework.master_crud_page import MasterCRUDPage


class StockInPage(MasterCRUDPage):
    """
    Nhập kho (StockIn).
    """

    ENTITY_NAME = "Stock In"
    ...
    DEFAULT_EXPORT_NAME = "stock_in.xlsx"

    def __init__(self):
        super().__init__(
            title="📥 Stock In",
            headers=self.HEADERS,
            search_placeholder="Search stock in...",
            service=StockInService(),
            importer=None,
            dialog_class=StockInDialog,
        )
    ...
    def load_records(self, keyword):
        return self.service.search_stock_in(keyword)

    @staticmethod
    def record_to_row(record):
        return [
            (
                record.stock_in_date.isoformat()
    ...
    def create_dialog(self, parent=None, record=None):
        return self.dialog_class(parent=parent, stock_in=record)
    ...
    def create_record(self, data):
        return self.service.create_stock_in(data)

    def update_record(self, record_key, data):
        return self.service.update_stock_in(record_key, data)

    def delete_record(self, record_key):
        return self.service.delete_stock_in(record_key)
